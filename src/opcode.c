 ...
 */
inline static uint8_t ADD_BB(CPU *cpu, uint8_t a, uint8_t b) { cpuSetFlag(cpu, FLAG_N, 0); cpuSetFlag(cpu, FLAG_C, (0xFF - a) < b ? 1 : 0); cpuSetFlag(cpu, FLAG_H, (0xF - (a & 0xF)) < (b & 0xF) ? 1 : 0); a += b; cpuSetFlag(cpu, FLAG_Z, !(a & 0xFF) ? 1 : 0); return a; }
inline static uint16_t ADD_BW(CPU *cpu, uint16_t a, uint8_t b) { cpuSetFlag(cpu, FLAG_Z, 0); cpuSetFlag(cpu, FLAG_N, 0); cpuSetFlag(cpu, FLAG_C, (0xFF - (a & 0xFF)) < b ? 1 : 0); cpuSetFlag(cpu, FLAG_H, (0xF - (a & 0xF)) < (b & 0xF) ? 1 : 0); return a + b; }
...
inline static uint8_t ADC(CPU *cpu, uint8_t a, uint8_t b) { uint8_t cf = cpuGetFlag(cpu, FLAG_C); cpuSetFlag(cpu, FLAG_C, (0xFF - a) < cf || (0xFF - (a + cf)) < b ? 1 : 0); cpuSetFlag(cpu, FLAG_H, (0xF - (a & 0xF)) < (cf & 0xF) || (0xF - ((a + cf) & 0xF)) < (b & 0xF) ? 1 : 0); a += b + cf; cpuSetFlag(cpu, FLAG_Z, !(a & 0xFF) ? 1 : 0); return a; }
inline static uint8_t SUB(CPU *cpu, uint8_t a, uint8_t b) { cpuSetFlag(cpu, FLAG_N, 1); cpuSetFlag(cpu, FLAG_C, a < b ? 1 : 0); cpuSetFlag(cpu, FLAG_H, (a & 0xF) < (b & 0xF) ? 1 : 0); a -= b; cpuSetFlag(cpu, FLAG_Z, !(a & 0xFF) ? 1 : 0); return a; }
inline static uint8_t SBC(CPU *cpu, uint8_t a, uint8_t b) { uint8_t cf = cpuGetFlag(cpu, FLAG_C); cpuSetFlag(cpu, FLAG_N, 1); cpuSetFlag(cpu, FLAG_C, a < cf || (a - cf) < b ? 1 : 0); cpuSetFlag(cpu, FLAG_H, (a & 0xF) < (cf & 0xF) || ((a + cf) & 0xF) < (b & 0xF) ? 1 : 0); a -= b - cf; cpuSetFlag(cpu, FLAG_Z, !(a & 0xFF) ? 1 : 0); return a; }
inline static uint8_t INC_B(CPU *cpu, uint8_t a) { a++; cpuSetFlag(cpu, FLAG_Z, !(a & 0xFF) ? 1 : 0); cpuSetFlag(cpu, FLAG_N, 0); cpuSetFlag(cpu, FLAG_H, !(a & 0xF) ? 1 : 0); return a; }
inline static uint16_t INC_W(uint16_t a) { return ++a; }
inline static uint8_t DEC_B(CPU *cpu, uint8_t a) { a--; cpuSetFlag(cpu, FLAG_Z, !(a & 0xFF) ? 1 : 0); cpuSetFlag(cpu, FLAG_N, 1); cpuSetFlag(cpu, FLAG_H, (a & 0xF) == 0xF ? 1 : 0); return a; }
...
inline static uint8_t DAA(CPU *cpu, uint8_t a) { uint8_t cf = cpuGetFlag(cpu, FLAG_C) || a > 0x99 ? 0x60 : 0; cf += cpuGetFlag(cpu, FLAG_H) || (a & 0xf) > 0x9 ? 0x6 : 0; int temp = cpuGetFlag(cpu, FLAG_Z) ? a - cf : a + cf; if (temp & 0xA0) { cpuSetFlag(cpu, FLAG_C, 1); } temp &= 0xFF; cpuSetFlag(cpu, FLAG_Z, !temp ? 1 : 0); cpuSetFlag(cpu, FLAG_H, 0); return temp; }
...
uint8_t cpuOpcode(CPU *cpu, MMU *mmu, uint8_t opcode)
{
	...
	switch (opcode) {
		...
		case 0x2F: /* CPL */
			cpu->regs.a = ~cpu->regs.a; cpuSetFlag(cpu, FLAG_N, 0); cpuSetFlag(cpu, FLAG_H, 0);
			return 4;
		...
		case 0x80: /* ADD A,B */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.b);
			return 4;

		case 0x81: /* ADD A,C */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.c);
			return 4;

		case 0x82: /* ADD A,D */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.d);
			return 4;

		case 0x83: /* ADD A,E */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.e);
			return 4;

		case 0x84: /* ADD A,H */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.h);
			return 4;

		case 0x85: /* ADD A,L */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.l);
			return 4;

		case 0x86: /* ADD A,(HL) */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, mmuReadByte(mmu, (cpu->regs.h << 8) + cpu->regs.l));
			return 10;

		case 0x87: /* ADD A,A */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, cpu->regs.a);
			return 4;
		...
		case 0x8E: /* ADC A,(HL) */
			cpu->regs.a = ADC(cpu, cpu->regs.a, mmuReadByte(mmu, (cpu->regs.h << 8) + cpu->regs.l));
			return 10;
		...
		case 0x96: /* SUB A,(HL) */
			cpu->regs.a = SUB(cpu, cpu->regs.a, mmuReadByte(mmu, (cpu->regs.h << 8) + cpu->regs.l));
			return 10;
		...
		case 0x9E: /* SBC A,(HL) */
			cpu->regs.a = SBC(cpu, cpu->regs.a, mmuReadByte(mmu, (cpu->regs.h << 8) + cpu->regs.l));
			return 10;
		...
		case 0xB8: /* CP B */
			SUB(cpu, cpu->regs.a, cpu->regs.b);
			return 4;

		case 0xB9: /* CP C */
			SUB(cpu, cpu->regs.a, cpu->regs.c);
			return 4;

		case 0xBA: /* CP D */
			SUB(cpu, cpu->regs.a, cpu->regs.d);
			return 4;

		case 0xBB: /* CP E */
			SUB(cpu, cpu->regs.a, cpu->regs.e);
			return 4;

		case 0xBC: /* CP H */
			SUB(cpu, cpu->regs.a, cpu->regs.h);
			return 4;

		case 0xBD: /* CP L */
			SUB(cpu, cpu->regs.a, cpu->regs.l);
			return 4;

		case 0xBE: /* CP (HL) */
			SUB(cpu, cpu->regs.a, mmuReadByte(mmu, (cpu->regs.h << 8) + cpu->regs.l));
			return 10;

		case 0xBF: /* CP A */
			SUB(cpu, cpu->regs.a, cpu->regs.a);
			return 4;
		...
		case 0xC6: /* ADD A,d8 */
			cpu->regs.a = ADD_BB(cpu, cpu->regs.a, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.pc++;
			return 10;
		...
		case 0xCE: /* ADC A,d8 */
			cpu->regs.a = ADC(cpu, cpu->regs.a, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.pc++;
			return 10;
		...
		case 0xD6: /* SUB A,d8 */
			cpu->regs.a = SUB(cpu, cpu->regs.a, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.pc++;
			return 10;
		...
		case 0xDE: /* SBC A,d8 */
			cpu->regs.a = SBC(cpu, cpu->regs.a, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.pc++;
			return 10;
		...
		case 0xE8: /* ADD SP,r8 */
			cpu->regs.sp = ADD_BW(cpu, cpu->regs.sp, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.pc++;
			return 16;
		...
		case 0xF1: /* POP AF */
			word = POP(mmu, &cpu->regs.sp); cpu->regs.a = word >> 8; cpu->regs.f = word & 0xFF;
			return 12;
		...
		case 0xF8: /* LD HL,SP+r8 */
			word = ADD_BW(cpu, cpu->regs.sp, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.h = word >> 8; cpu->regs.l = word & 0xFF; cpu->regs.pc++;
			return 12;
		...
		case 0xFE: /* CP d8 */
			SUB(cpu, cpu->regs.a, mmuReadByte(mmu, cpu->regs.pc)); cpu->regs.pc++;
			return 10;
